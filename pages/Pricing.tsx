...
import { INITIAL_PRICING_PLANS } from '../constants';
...
const PricingCard: React.FC<{ plan: PricingPlan }> = ({ plan }) => {
  ...
  return (
    <div className={`bg-white rounded-[3rem] p-12 border shadow-sm relative flex flex-col transition-all duration-300 ${plan.isPopular ? 'border-[#014034] ring-2 ring-[#014034] lg:scale-105 z-10' : 'border-gray-100'}`}>
      {plan.isPopular && <div className="absolute top-0 right-1/2 translate-x-1/2 -translate-y-1/2 bg-[#014034] text-white text-xs font-bold uppercase px-6 py-2 rounded-full shadow-lg">Most Popular</div>}
      <div className="mb-8">
      ...
      <Link to="/get-quote" className={`w-full py-5 rounded-2xl font-extrabold text-lg flex items-center justify-center space-x-3 transition-all ${plan.isPopular ? 'bg-[#014034] text-white hover:bg-[#00332a]' : 'bg-gray-100 text-[#014034] hover:bg-gray-200'}`}>
  ...
  return (
    <div className="pt-32 pb-24 bg-gray-50">
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">{INITIAL_PRICING_PLANS.map((p) => <PricingCard key={p.id} plan={p} />)}</div>
        <div className="mt-20 text-center">
          <p className="text-gray-500 text-lg">Need something tailored? <Link to="/contact" className="text-[#00695c] font-bold hover:underline">Contact sales</Link></p>
        </div>
      </div>
    </div>
  );
